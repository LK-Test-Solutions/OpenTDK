...
public class RT_CSVContainer_setValues extends BaseRegression {
	...
	public void runTest() {
		...
		dc.tabInstance().setValue("Ort", "Amberg");
		testResult(dc.tabInstance().getValue("Ort"), "setValue(header,value) - value", "Amberg");

		dc.tabInstance().setValue("Firma", 2, "Neotys");
		testResult(dc.tabInstance().getValue("Firma", 2), "setValue(header,index,value) - value", "Neotys");

		Filter fltr = new Filter();
		dc.tabInstance().setValue("Ort", "Freiburg", fltr);
		testResult(dc.tabInstance().getValue("Ort"), "setValue(header,value,filter) - value", "Freiburg");

		dc.tabInstance().setValue("Strasse", 1, "Teststrasse", new Filter());
		testResult(dc.tabInstance().getValue("Strasse", 1), "setValue(header,index,value,filter) - value", "Teststrasse");

		fltr.clear();
		fltr.addFilterRule("Firma", "GmbH", EOperator.CONTAINS);
		dc.tabInstance().setValues("Firma", "LK Test Solutions AG", fltr);
		testResult(dc.tabInstance().getValuesAsDistinctedList("Firma").get(2), "setValues(header,value,filter) - values equal", "LK Test Solutions AG");

		dc.tabInstance().setValues("Email", new int[] { 1 }, "fme@lk-test.de", new Filter());
		testResult(dc.tabInstance().getValue("Email", 1), "setValues(header,indices,value,filter) - value", "fme@lk-test.de");
